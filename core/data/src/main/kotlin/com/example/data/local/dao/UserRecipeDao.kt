...
interface UserRecipeDao {
    ...
    @Query("DELETE FROM want_to_cook WHERE userId = :userId")
    suspend fun clearWantToCook(userId: String)

    @Query("DELETE FROM cooked WHERE userId = :userId")
    suspend fun clearCooked(userId: String)

    @Query("DELETE FROM user_recipes WHERE userId = :userId")
    suspend fun clearUserRecipes(userId: String)

    suspend fun clearAllUserData(userId: String) {
        clearWantToCook(userId)
        clearCooked(userId)
    ...
    @Query("SELECT COUNT(*) FROM cooked WHERE userId = :userId")
    fun getCookedCountFlow(userId: String): Flow<Int>

    @Query("SELECT COUNT(*) FROM user_recipes WHERE userId = :userId")
    fun getUserRecipesCountFlow(userId: String): Flow<Int>
}